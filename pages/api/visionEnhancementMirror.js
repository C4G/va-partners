...
export default async function handler(req, res) {
  ...
  await updateUserLastModified("visionEnhancementMirror", req.method, session.user.email);
  if (req.method === "POST") {
...
async function addData(req, res) {
  ...
  try {
    const vision_Enhancement_Mirror = await prisma.vision_Enhancement_Mirror.update(update);
