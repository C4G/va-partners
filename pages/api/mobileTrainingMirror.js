...
export default async function handler(req, res) {
  ...
  await updateUserLastModified("mobileTrainingMirror", req.method, session.user.email);
  if (req.method === "POST") {
...
async function addData(req, res) {
  ...
  try {
    const mobile_Training_Mirror = await prisma.mobile_Training_Mirror.update(update);
