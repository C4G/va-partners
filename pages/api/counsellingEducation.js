...
export default async function handler(req, res) {
  ...
  await updateUserLastModified("counsellingEducation", req.method, session.user.email);
  if (req.method === "POST") {
...
async function deleteData(req, res) {
  ...
  try {
    ...
    res.status(500).json({ error: "Error deleting Counseling data" + error, success: false });
  }
}
