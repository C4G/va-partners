  ...
  if (checkbox) {
    return values.map((value) => (
      <MenuItem key={value} value={value} style={{ whiteSpace: "normal" }}>
        ...
        <ListItemText primary={<Typography align="left">{value}</Typography>} />
      </MenuItem>
    ));
  } else {
    return values.map((value) => (
...
export const createOptionMenu = (values, subheadings, indices, checkbox, selected = []) => {
  const fullMenu = createMenu(values, checkbox, selected);
  const optionGroups = [];
  for (var i = 0; i < subheadings.length; i++) {
    optionGroups.push(<ListSubheader key={subheadings[i]}>{subheadings[i]}</ListSubheader>);
    optionGroups.push(fullMenu.slice(indices[i], indices[i + 1]));
  }
  return optionGroups;
};

export const isNotNullBlankOrUndefined = (variable) => variable !== null && variable !== undefined && variable !== "";
